...
function parseNumberFlag(rawValue: string | undefined, flagName: string): number {
    if (!rawValue) {
...
function parseOptions(argv: string[]): Options {
    ...
    for (const arg of argv) {
        ...
        if (arg.startsWith('--batch-size=')) {
            options.batchSize = parseNumberFlag(arg.slice('--batch-size='.length), '--batch-size');
            continue;
        }

        if (arg.startsWith('--max-batches=')) {
            const value = parseNumberFlag(arg.slice('--max-batches='.length), '--max-batches');
...
async function run(): Promise<void> {
    ...
    while (options.maxBatches === null || batches < options.maxBatches) {
        ...
        if (!options.apply) {
            ...
            const updates = rows.map(row => prisma.feedback.updateMany({
                where: {
                    id: row.id,
