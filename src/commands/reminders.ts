...
export default {
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(subcommand =>
            subcommand
                .setName('run-now')
                .setDescription('Runs the raid attendance reminder cycle for this server immediately'),
        ),
    async execute(interaction: ChatInputCommandInteraction, context: AppContext) {
        ...
        const result = await runGuildRaidAttendanceReminderCycle(context, interaction.guildId);

        if (result.skipped) {
            const content = result.skippedReason === 'settings_missing'
